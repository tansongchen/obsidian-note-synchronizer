...
export class NoteManager {
  ...
  validateNote(file: TFile, frontmatter: FrontMatterCache, content: string, media: EmbedCache[] | undefined, noteTypes: Map<number, NoteTypeDigest>): [Note | undefined, MediaNameMap[] | undefined] {
    if (!frontmatter.hasOwnProperty("mid") || !frontmatter.hasOwnProperty("nid") || !frontmatter.hasOwnProperty("tags")) return [undefined, undefined];
  ...
  parseFields(title: string, noteType: NoteTypeDigest, body: string[], media: EmbedCache[] | undefined): [Record<string, string> | undefined, MediaNameMap[] | undefined] {
    ...
    for (const line of body) {
      if (line.slice(0, headingLevel + 1) === "#".repeat(headingLevel) + " ") {
        ...
        if (media && mediaCount < media.length && line.includes(media[mediaCount].original) && this.validateMedia(media[mediaCount].link)) {
          let mediaName = line.replace(media[mediaCount].original, media[mediaCount].link.split("/").pop() as string);
  ...
  validateMedia(mediaName: string) {
    return [...PICTURE_EXTENSION, ...VIDEO_EXTENSION].includes(mediaName.split(".").pop() as string);
  }
