...
abstract class State<K, V, I = undefined> extends Map<K, V> {
    ...
    async change(state: Map<K, V>, extra: Map<K, I> = new Map<K, I>()) {
        const _keys = [...this.keys()];
        const keys = [...state.keys()];
        for (const key of keys) {
            await this.update(key, state.get(key), extra.get(key));
...
export class NoteTypeState extends State<number, NoteTypeDigest> {
    ...
    delete(key: number) {
        const templatePath = `${this.templateFolderPath}/${this.get(key).name}.md`;
...
export class NoteState extends State<number, NoteDigest, Note> {
    ...
    async handleAddNote(note: Note) {
        const ankiNote = {
            ...
            modelName: this.plugin.noteTypeState.get(note.mid).name,
